...
class solver_decoder:
    ...
    def decode(self,chromosome,rewrite):  
        ...
        #Restrição de flow
        c2 = m.addConstrs((gb.quicksum(xij[i,j] for i in fabricas_escolhidas) >= gb.quicksum(sjk[j,k] for k in np.arange(K)))
                           for j in depositos_escolhidos)
        #Restrição capacidade das plantas 
        c3 = m.addConstrs((gb.quicksum(xij[i,j] for j in depositos_escolhidos)<= self.instance.plant_capacity[i]* yi[i]) 
                          for i in fabricas_escolhidas)
        #Restrição capacidade dos depositos
        c4 = m.addConstrs((gb.quicksum(sjk[j,k] for k in np.arange(K))<= self. instance.stelite_capacity[j] * zj[j])
                           for j in depositos_escolhidos)
        m.optimize()
        if m.SolCount >= 1:
            ret = m.ObjVal
